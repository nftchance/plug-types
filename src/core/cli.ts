...
program
	.name('plug')
	.description('A tool for generating code for the Plug framework.')
...
program
	...
	.action(async options => {
		...
		if (configPath) {
			console.info(
				pc.blue(
					`* Found Plug configuration file at: \n\t ${pc.gray(
						configPath
					)}`
				)
			)
		...
		if (options.config) {
			...
			outPath = resolve(rootDir, `plug.config.${extension}`)
		}
		...
		if (isUsingTypescript) {
			...
			content = dedent(`
                ...
                /** @type {import('@nftchance/plug-types').config} */
		...
		console.info(
			pc.green(
				`✔︎ Generated Plug configuration file at: \n\t ${pc.gray(
					outPath
				)}`
			)
		)
...
program
	...
	.action(async options => {
		...
		if (configPath) {
			...
			console.info(
				pc.yellow(
					`! Could not find Plug configuration file. Using default.`
				)
			)
